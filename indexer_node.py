...
def indexer_process():
    ...
    while True:
        ...
        try:
            ...
            comm.send(f"Indexer {rank} - Indexed content from Crawler {source_rank}", dest=0, tag=99)  # Status update

        except Exception as e:
            logging.error(f"Indexer {rank} error indexing content from Crawler {source_rank}: {e}")
            comm.send(f"Indexer {rank} - Error indexing: {e}", dest=0, tag=999)  # Report error to master
